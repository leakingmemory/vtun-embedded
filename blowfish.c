...
static inline uint32_t blowfish_f(BlowfishContext *ctx, uint32_t input) {
    ...
    uint8_t b1 = input & 0xFF;
    uint32_t h = ctx->s[b4] + ctx->s[0x100 + b3];
...
void blowfish_encrypt_8bytes_le(BlowfishContext *ctx, void *block) {
    uint32_t l = le32toh(*((uint32_t *)block));
    uint32_t r = le32toh(*((uint32_t *)(block + 4)));
    blowfish_encrypt_lr(ctx, &l, &r);
    *((uint32_t *)block) = htole32(l);
    *((uint32_t *)(block + 4)) = htole32(r);
}

void blowfish_decrypt_8bytes_le(BlowfishContext *ctx, void *block) {
    uint32_t l = le32toh(*((uint32_t *)block));
    uint32_t r = le32toh(*((uint32_t *)(block + 4)));
    blowfish_decrypt_lr(ctx, &l, &r);
    *((uint32_t *)block) = htole32(l);
    *((uint32_t *)(block + 4)) = htole32(r);
}

void blowfish_encrypt_8bytes_ecb(BlowfishContext *ctx, void *block) {
    uint32_t l = be32toh(*((uint32_t *)block));
    uint32_t r = be32toh(*((uint32_t *)(block + 4)));
    blowfish_encrypt_lr(ctx, &l, &r);
    *((uint32_t *)block) = htobe32(l);
    *((uint32_t *)(block + 4)) = htobe32(r);
}
