...
tar -czvf vtun-embedded-$version.tar.gz freebsd generic linux openbsd packages scripts svr4 *.m4 auth.c client.c lfd_encrypt.c lfd_legacy_encrypt.c lfd_lzo.c lfd_shaper.c lfd_zlib.c lib.c linkfd.c llist.c lock.c main.c netlib.c server.c tunnel.c vtun.c md5.c blowfish.c *.h *.l *.y ChangeLog config.guess *.in config.sub configure Credits FAQ install-sh README README.LZO README.OpenSSL README.Setup README.Shaper TODO vtun.drivers vtunemd.8 vtunemd.conf vtunemd.conf.5
