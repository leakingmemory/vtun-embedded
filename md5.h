...
static void md5_tohex(md5_hex_str *result, md5_hash md) {
	int i;
	for (i = 0; i < 16; i++) {
...
static void md5(md5_hash *result, void *buf, size_t len) {
	Md5Context ctx;
	md5_init(&ctx);
	return md5_final(result, &ctx, buf, len);
}

static void md5_hex(md5_hex_str *str, void *buf, size_t len) {
	md5_hash md;
	md5(&md, buf, len);
	md5_tohex(str, md);
}
